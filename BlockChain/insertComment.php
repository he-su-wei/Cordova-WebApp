<?php
    ...
    // if(isset($_POST)){
        ...
        $path = "photos";
        //呼叫函式
        base64_image_content($storeName, $account, $comment, $base64_image_content, $path, $conn);

        //base64格式編碼轉換為圖片並儲存對應資料夾
        function base64_image_content($storeName, $account, $comment, $base64_image_content,$path, $conn){
            //匹配出圖片的格式
            if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_image_content, $result)){
                ...
                if (file_put_contents($new_file, base64_decode(str_replace($result[1], '', $base64_image_content)))){
                    ...
                    $image = '/'.$new_file;
                    //路徑儲存到資料庫
                    $sql = "INSERT INTO $storeName (account, comment, img) VALUES ('$account', '$comment', '$image')";
                    // echo $sql;
